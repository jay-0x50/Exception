...
class BOSS_RAID_API UBRBossStatusWidget : public UUserWidget
{
	GENERATED_BODY()
...
protected:
	...
	UFUNCTION(BlueprintImplementableEvent, Category="BossRaid|Boss UI", meta=(DisplayName="Clear Bosses"))
	void BP_ClearBosses();

	UFUNCTION(BlueprintImplementableEvent, Category="BossRaid|Boss UI", meta=(DisplayName="Set Boss Count"))
	void BP_SetBossCount(int32 BossCount);

	UFUNCTION(BlueprintImplementableEvent, Category="BossRaid|Boss UI", meta=(DisplayName="Set Boss HP"))
	void BP_SetBossHP(int32 BossIndex, const FText& BossName, float CurrentHP, float MaxHP, float NormalizedHP);

	UFUNCTION(BlueprintImplementableEvent, Category="BossRaid|Boss UI", meta=(DisplayName="Set Boss Groggy"))
	void BP_SetBossGroggy(int32 BossIndex, float CurrentGroggy, float MaxGroggy, float NormalizedGroggy);

	UFUNCTION(BlueprintImplementableEvent, Category="BossRaid|Boss UI", meta=(DisplayName="Set Boss Groggy State"))
